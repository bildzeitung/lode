...
| `jira_projects` | `[]` (empty) | JIRA project-key allowlist gating BARE issue-key detection — a plain `PROJ-123` in note prose, not just a pasted `/browse/` permalink. Empty (default) means bare-key detection is off, byte-for-byte today's behavior. See "Bare JIRA issue keys" below. |
