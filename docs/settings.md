...
| `jira_base_url` | `""` (empty) | API base override, e.g. `https://acme.atlassian.net`. Empty means infer from the pasted link at detection time. A non-empty value must be a well-formed `http(s)` URL — a malformed one fails validation at `Settings()` construction. **Mandatory** (not just an override) for bare-issue-key detection — a bare key has no host of its own to infer a base from. Same class as lode-0teo: the derived docs/settings.md jira_base_url row says "detection below" pointing at prose that never derives; the new gate regex does not match this shape. Regenerate docs/settings.md via scripts/generate_derived_docs.py when applying. |
| `confluence_base_url` | `""` (empty) | Same shape as `jira_base_url`, for Confluence. |
| `jira_projects` | `[]` (empty) | JIRA project-key allowlist gating BARE issue-key detection — a plain `PROJ-123` in note prose, not just a pasted `/browse/` permalink. Empty (default) means bare-key detection is off, byte-for-byte today's behavior. See the "Bare JIRA issue keys" section of externals.md for the full detection rules and rationale. |
| `LODE_JIRA_TOKEN` env var / `jira_token` (config.toml fallback) | unset / `""` | JIRA Cloud API token. Resolved **env-var PRIMARY**: `LODE_JIRA_TOKEN` is checked first, then the `jira_token` key in `config.toml` as fallback. No secret is required to live in `config.toml`. **The raw value is never logged, echoed, or shown by `lode config`** — `secret=True` shows only a presence indicator in the knob table, never the value: a bracketed `REDACTED` marker when either source resolves, a bracketed `unset` marker when neither does. |
...
| `no_egress_scopes` | `[]` | Declarative no_egress SCOPE rules: a list of `{source_type, match}` rules evaluated live against a candidate `(external_id, source_type)` pair, covering externals no row exists for yet — unlike the per-row `no_egress` flag, which can only mark a resource already captured. |
