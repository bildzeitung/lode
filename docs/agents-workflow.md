...
only the *machinery* (`scripts/recycled-worktree-guard.sh`, lode-ivth) and the *quick-card row*
(`coding.md`'s and `code-reviewer.md`'s own quick cards, consolidated to one canonical version below —
`land-review.md` carries no such table) are safe to consolidate (lode-zt62).** `coding.md`,
