...
- **The docs lookup index is where "what did we decide about X" starts.** `docs/decisions.md` alone
  ...
  chunk); when that returns nothing, this docs index query retries with `OR` semantics over the
  same terms and marks the printed rows `[fallback: OR match]` instead of printing "No results."
