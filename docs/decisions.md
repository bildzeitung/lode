...
- **`/land`'s worktree backstop 1 predicate widened from "merged into trunk" to "merged into trunk OR
  ...
  *explicit* lock on its producer build worktree, per lode-oqr, because it unlocks again at its first
  ...
  has since landed (6591ba9)**, replacing the "merged"/"captured" proxy with a real dirty-tree guard
  that gates both arms — see the entry below and the loop's own inline comments in
  `.claude/skills/land/SKILL.md`. Its build correctly relied on the harness lock rather than
  ...
  on, would not have been released if the agent died, reopening the very indefinite leak this ticket
  closes.
