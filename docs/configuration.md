...
| Q&A call timeout (`qa_call_timeout_s`) | runtime | `300s` | Per-call client-side timeout for the Q&A synthesis call **only** (`qa.py`'s `structured_call`), split off `llm_call_timeout_s` (`lode-wfyx`, since renamed `enrich_call_timeout_s` in `lode-7y6s`) — [below](#qa-call-timeout-split-from-llm_call_timeout_s-decided-lode-wfyx). **Derived, not a measured p95.** Does not reach `enrich.py`'s three call sites — those stay on `enrich_call_timeout_s` (120s) unchanged. The derivation, the retained SDK retry-on-timeout it was chosen alongside, and the `ModelTier.max_tokens` override that invalidates it are all in the write-up linked above, deliberately not restated here. |
...
> about, was subsequently renamed `enrich_call_timeout_s` — the general name
> covered only the enrichment call sites once this split landed, and the
> mismatch is a real, user-facing footgun (`llm_call_timeout_s` silently
> doing nothing for a slow `lode ask`). The write-up below is left as
> written at decision time; read every `llm_call_timeout_s` reference here
> as `enrich_call_timeout_s`.
