...
| Knob | Kind | Default | Notes |
|---|---|---|---|
| CLI theme (`[cli.theme]`) | runtime | absent | Optional, `extra="forbid"` section. Absent leaves every current default byte-identical (`lode.cli.CLI_STYLES`). `[cli.theme.styles]` overrides the CLI's semantic rich style names (fixed key set: `note_id`, `date`, `warn`, `danger`, `ok`, `table_header` — `_` stands in for the literal `.` in `table.header`, which TOML cannot key). Values are rich **style** strings, not colour-only ones (`"bold red"`, `"dim"` — `CLI_STYLES`'s own defaults are style strings), parsed at config load with `rich.style.Style.parse`; a bad value or unknown key is a load error naming the offending key. Resolved and applied once per invocation in `lode.cli.main()`, so it covers every subcommand — with `lode status` and `lode theme` exempted (the `_CONFIG_OPTIONAL_COMMANDS` set), each keeping its own broken-`config.toml` survival contract (unstyled rather than failing) instead of erroring. Run `lode theme export` to print the fully-resolved effective theme (TUI and CLI sections alike) as ready-to-paste TOML — it is itself best-effort against a broken config, falling back to built-in defaults with a stderr warning rather than failing, since it is the recovery tool for exactly that config. Design record: [decisions.md](decisions.md)'s `lode-mk9j` entry (2026-08-18) and its `lode-jjol` entry (2026-08-19, the two-command exemption set). |
...
| `jira_base_url` | runtime | `""` (empty) | API base override, e.g. `https://acme.atlassian.net`. Empty means infer from the pasted link at detection time. A non-empty value must be a well-formed `http(s)` URL — a malformed one fails validation at `Settings()` construction. **Mandatory** (not just an override) for bare-issue-key detection (the `jira_projects` row below, and the "Bare JIRA issue keys" section of [externals.md](externals.md#bare-jira-issue-keys-lode-2o45)) — a bare key has no host of its own to infer a base from. Same class as lode-0teo: the derived docs/settings.md jira_base_url row says "detection below" pointing at prose that never derives; the new gate regex does not match this shape. Regenerate docs/settings.md via scripts/generate_derived_docs.py when applying. |
| `confluence_base_url` | runtime | `""` (empty) | Same shape as `jira_base_url`, for Confluence. |
| `jira_projects` | runtime | `[]` (empty) | JIRA project-key allowlist gating BARE issue-key detection (`lode-2o45`) — a plain `PROJ-123` in note prose, not just a pasted `/browse/` permalink. Empty (default) means bare-key detection is off, byte-for-byte today's behavior. See the "Bare JIRA issue keys" section of [externals.md](externals.md#bare-jira-issue-keys-lode-2o45) for the full detection rules and rationale. |
| `LODE_JIRA_TOKEN` env var / `jira_token` (config.toml fallback) | runtime | unset / `""` | JIRA Cloud API token. Resolved **env-var PRIMARY**: `LODE_JIRA_TOKEN` is checked first, then the `jira_token` key in `config.toml` as fallback. No secret is required to live in `config.toml`. **The raw value is never logged, echoed, or shown by `lode config`** — `secret=True` (`src/lode/config.py::_knob`) shows only a presence indicator in the knob table, never the value: a bracketed `REDACTED` marker when either source resolves, a bracketed `unset` marker when neither does. |
...
| `atlassian_connect_retries` | build | `1` | Connection-**establishment** retries (`httpx2.HTTPTransport(retries=N)`, `lode-lq9u`) for the JIRA and Confluence connector HTTP clients only — never a sent request, so retrying is idempotency-safe. Absorbs a one-off connect blip in-process instead of costing a full `TransientFetchError` -> work-queue retry round-trip. Deliberately **not** applied to `webfetch.py`'s ask-path `GuardedHttpxFetcher`, whose one-hop verify-the-peer-before-reading design (`lode-xwah`) must not get a silent transport-level reconnect to a different peer than the one verified. |
...
| `no_egress_scopes` | runtime | `[]` | Declarative no_egress SCOPE rules (lode-35nu.11.8): a list of `{source_type, match}` rules evaluated live against a candidate `(external_id, source_type)` pair, covering externals no row exists for yet — unlike the per-row `no_egress` flag, which can only mark a resource already captured. |
