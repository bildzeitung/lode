...
`lode-crh8`'s DB-invalidation scoping also covers the **enrichment LLM** (`enrichment_llm`, [Models](#models) above) — its output (tags, entities, summaries, inferred edges) persists into `annotations`/`edges` the same way the embedder's output persists into `embeddings`, so a silent model change is the same class of problem for enrichment as for embedding. Whether an Anthropic model ID has a dated-snapshot form to pin against is **not a fixed rule** — it varies per model and shifts as the lineup changes, so check the authoritative model table (e.g. the `claude-api` skill's model catalog, or `GET /v1/models/{id}`) rather than assuming from a prior generation or a sibling model. As of writing, `qa_llm`'s and `qa_think_harder_llm`'s defaults (`claude-sonnet-4-6`, `claude-opus-5`) have **no dated-snapshot form** — the bare/marketing ID *is* the complete stable identifier, and appending a date suffix 404s — but `enrichment_llm`'s default, `claude-haiku-4-5`, **does**: `claude-haiku-4-5-20251001` resolves as a distinct, pinnable identifier alongside the bare ID. That means a PIN axis is not uniformly absent the way this section previously claimed — it genuinely exists today for the enrichment LLM's default model, unlike for the Q&A tiers' defaults. Whether that should change enrichment's DETECT-only design is an open question, not settled here: see [decisions.md](decisions.md) (search "lode-sdjb") for the tracking entry. For any tier whose configured model genuinely has no dated-snapshot form, there is nothing more specific to pin against, and the mechanism collapses to record + detect:
