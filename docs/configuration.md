...
| Batch collect failure budget (`batch_collect_failure_budget`) | runtime | `5` | Consecutive `collect_enrich_batch()` failures (the poll call itself raising, not an individual result's errored/expired/canceled outcome) tolerated for one `batch_handle` before its still-`running` jobs are dead-lettered outright — no final salvage attempt. Resets to 0 on any poll that doesn't raise. Closes the last of the three poison-pill axes `_batch_collect_enrich`'s per-handle isolation left open (`lode-u6he`; [storage.md](storage.md#transient-vs-permanent-job-failures--pinned-lode-9yy)). |
