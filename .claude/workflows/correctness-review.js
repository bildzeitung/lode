export const meta = {
  ...
  whenToUse:
    'NOT on any automatic path — DELIBERATE MANUAL invocation only (a one-off review of a hairy diff, or the specs/11-specs/13 runbooks), from a Workflow-capable main session; a dispatched coding or code-reviewer subagent does not reach the Workflow tool at all (verified empirically, lode-905v). /code does NOT invoke this: lode-rlyx removed it from Phase 2 on measured cost, and the code-reviewer\'s own reasoned pass IS the correctness review — do not wire it back without new evidence it beats that pass per token (docs/decisions.md). Requires args {refRange}: a git ref range/comparison that `git diff` accepts directly (e.g. "trunk...HEAD" for a live review, or a historical "<sha1>...<sha2>" for a retrospective run) — both ends must already be reachable commits; no working-tree checkout is performed, so the caller does not need to be sitting on any particular branch.',
  phases: [
