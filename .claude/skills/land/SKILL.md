...
`land/<id>--<worktree-dir>`, or any other name) or **detached** alike — regardless of what any ticket's
now-vestigial `review_worktree` pointer claims (see above: nothing reads that field). lode-jiyk unified
what were originally two separate **worktree** sweeps here: an early one keyed on branch **name**
(`lode-r78`), and a later one keyed directly on
