...
label**, so `bd list --label land-escalated` can reach empty. `/land` itself never performs any of
these three; it only ever *sets* `land-escalated` (above). Resolution is a human action taken
outside a `/land` pass — typically at `bd show <id>` time — not something `/land` does for itself.
There are exactly three exits:

### (a) Land as-is — materialize the fix first, then re-enter the queue

A branch reaches `land-escalated` because `land-review` hit a genuine ambiguity it couldn't settle
on its own (an arguable "done", an unclear acceptance criterion). If the human decides the branch
**should** land, swapping the label back to `ready-for-land` with nothing else changed is **not a
complete transition**: `/land`'s next pass just re-dispatches `land-review`, which hits the *same*
ambiguity and escalates again — an infinite escalate↔ready loop.

The swap is only valid once the human has **materialized the decision in the ticket itself** —
edit the acceptance criteria / description so the ambiguity `land-review` flagged no longer exists
— *then* swap the label. `land-review` stays authoritative on the next pass; there is deliberately
**no "human-blessed" bypass label** that skips semantic re-review (forcing a land past a
`land-review` objection is an out-of-band manual act, not a designed fast-path):

```bash
rtk bd update <id> --description="<revised, unambiguous acceptance/description>"   # remove the ambiguity land-review flagged
...
All three end the same way: **`land-escalated` is gone**, so a queue like `/sweep`'s can actually
drain rather than growing monotonically.
