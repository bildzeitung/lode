...
9. **Record mistakes in MISTAKES.md — autonomously, without waiting to be told.** Every agent, at
   every stage of the workflow (build, review, land, sweep, audit, …), appends to MISTAKES.md the
   moment it discovers a qualifying mistake — it does not wait for a human to notice or order the
   write. **Qualifying bar (stated once, here — every other instruction file points back to this
   ...
   `land-review`, run in a disposable worktree that never commits — reports the finding to whoever
   dispatched it, which files on its behalf; `/land`, running on `trunk` in the main checkout, treats
   this file as a narrow, explicit exception to its "report the patch, not the gap" rule).
