...
4. Re-create the deliberately **user-scope** (`~/.claude/settings.json`) pieces as wanted — these do NOT travel: the rtk PreToolUse hook (`{"matcher": "Bash", "hooks": [{"type": "command", "command": "rtk hook claude"}]}`), `"defaultMode": "acceptEdits"`, model choice, personal statusline. A related but distinct per-machine knob lives in the **project-scoped** `.claude/settings.local.json` instead (gitignored, so it doesn't travel either): `/code`'s concurrency cap (`LODE_CODE_MAX_CONCURRENT_AGENTS`) is derived once from this machine's `/proc/meminfo` + `nproc` and cached there. It will **not** auto-update if the machine changes (more RAM, a different box, a raised WSL2 memory limit) — just ask Claude to recompute it after any hardware/VM-size change. Full rationale: [`docs/agents-workflow.md` — Concurrency cap](docs/agents-workflow.md#concurrency-cap-lode-2cf).
