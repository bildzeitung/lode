...
- **(E) Body representation:** request the server-rendered ``body.view``
  ...
  **existing** :func:`lode.webfetch._extract` (trafilatura) — the identical
  ...
  as ``raw_payload`` for provenance, mirroring :attr:`~lode.webfetch.FetchResult.raw_html`'s field on the web leg even though what it holds
...
- **(C) Classification:** every HTTP outcome is classified via the shared,
  ...
  429/any 5xx/network/timeout raise :class:`~lode.webfetch.TransientFetchError`, riding the worker's existing attempts/backoff/
...
def fetch_confluence_page(
    ...
    *,
    ...
    Pure function, no DB writes — mirrors :func:`lode.webfetch.fetch_and_extract`'s contract exactly, so :func:`lode.externals.ingest_fetch_result` consumes the result unchanged. See the module
    ...
    3. Classify the response status via :func:`~lode.fetch_outcome.classify_http_status`. A conforming ``fetcher`` already raises
