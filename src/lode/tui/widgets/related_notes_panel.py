...
class RelatedNotesPanel(Static):
    ...
    def on_unmount(self) -> None:
        ...
        running at all, for the part that *can* be stopped. The pass itself
        runs via ``asyncio.to_thread`` (:meth:`_search_related`), and that
        thread work is uncancellable: once a pass has entered the thread,
        this only discards its eventual result -- the thread keeps running to
        completion regardless. See docs/tui.md's "RelatedNotesPanel's
        background pass" section for the tolerate-the-straggler decision that
    ...
    def _cancel_related_pass(self) -> None:
        ...
        ``asyncio.to_thread`` work it awaits is uncancellable: cancelling
        here discards the result of a pass already running in its thread,
        it does not stop that thread -- it keeps executing to completion on
        its own schedule regardless. See docs/tui.md's "RelatedNotesPanel's
        background pass" section for the tolerate-the-straggler decision
        that accepts this residual.
        """
        if self._related_timer is not None:
            self._related_timer.stop()
            self._related_timer = None
